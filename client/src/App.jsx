...
function RouteLoadingFallback({ themeColor, onVisibleChange = null }) {
  ...
  const content = (
    ...
    >
      <div className="flex flex-col items-center gap-3 text-center text-indigo-700 dark:text-indigo-300">
...
export default function App() {
  ...
  function refreshThemeColorForSafari(color, allowScrollNudge = true) {
    ...
    const nudgeColor = color === '#ffffff' ? '#fefefe' : '#090d19'
  ...
  function getThemeColor(nextIsDark, nextRoute = route) {
    ...
    if (nextIsDark) {
      return onChatRoute ? '#0f172a' : '#0a0e1a'
    }
    return '#ffffff'
  }
  ...
  const appShellClass = isAuthRoute
    ? 'min-h-screen bg-gradient-to-b from-white via-indigo-50/70 to-white text-slate-900 transition-colors duration-300 dark:bg-gradient-to-b dark:from-indigo-950 dark:via-slate-950 dark:to-slate-900 dark:text-slate-100'
  ...
  return (
    <LanguageProvider>
      ...
      >
        ...
        {isAuthRoute ? (
          <>
            <div className="pointer-events-none fixed -top-40 left-1/2 h-96 w-96 -translate-x-1/2 rounded-full bg-indigo-400/30 blur-[130px]" />
