...
export default function InvitePage({
  ...
  return (
    <section
      ref={cardRef}
      className={`relative w-full max-w-md rounded-3xl border border-indigo-200/70 bg-white/80 p-6 shadow-2xl shadow-indigo-500/10 backdrop-blur dark:border-white/5 dark:bg-slate-900/80 sm:p-8 ${
    ...
    >
      ...
      <div className="mt-4 sm:mt-6">
        {loading ? (
          ...
          <>
            <div className="rounded-2xl border border-indigo-200/80 bg-indigo-50/60 p-4 text-center dark:border-indigo-500/30 dark:bg-indigo-500/10">
              {groupAvatarUrl ? (
                <img
                  src={groupAvatarUrl}
                  alt={groupName}
              ...
              <p className="w-full text-center">
                <span
                  className={`mx-auto block w-fit max-w-full truncate text-base font-semibold text-indigo-800 dark:text-indigo-200 ${
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              {!alreadyMember ? (
                <button
                  ...
                  onClick={handleJoin}
                  disabled={joining}
                  className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-indigo-500 px-4 text-sm font-semibold text-white transition hover:bg-indigo-400 disabled:opacity-70"
                >
                  {joining ? (
