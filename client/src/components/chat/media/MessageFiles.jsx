    ...
    return (
      <div className="inline-flex w-max max-w-full items-center gap-2 rounded-xl border border-indigo-200/70 bg-white px-3 py-2 text-xs text-slate-900 transition hover:border-indigo-300 hover:bg-white/90 hover:shadow-[0_0_16px_rgba(16,185,129,0.18)] dark:border-indigo-500/30 dark:bg-slate-900/50 dark:text-slate-200 dark:hover:bg-slate-900/70 dark:hover:shadow-[0_0_16px_rgba(16,185,129,0.14)]">
        <button
          ...
          onClick={togglePlay}
          disabled={!canPlay}
          className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-indigo-200 bg-indigo-100 text-indigo-700 transition hover:border-indigo-300 hover:bg-indigo-200/70 disabled:cursor-not-allowed disabled:opacity-60 dark:border-indigo-500/30 dark:bg-indigo-500/10 dark:text-indigo-200"
          aria-label={
            ...
            {peaks.map((height, idx) => {
              ...
              return (
                <span
                  ...
                  className={`w-[4px] rounded-full transition-colors duration-150 ${isPlayed ? "bg-indigo-600" : "bg-indigo-200/90 dark:bg-indigo-500/30"}`}
                  style={{ height: `${barHeight}px` }}
                />
              );
            })}
          </div>
          <span className="text-[10px] text-slate-900/80 dark:text-indigo-200/80">
            {canPlay
      ...
      {files.map((file, fileIndex) => {
        ...
        if (isImage && file.url) {
          return (
            <button
              ...
              className="relative block w-full overflow-hidden rounded-xl border border-indigo-200/70 bg-white/70 dark:border-indigo-500/30 dark:bg-slate-900/50"
            >
              <div className={imageFrameClass} style={mediaFrameStyle}>
                ...
                {isDesktop && !thumbLoaded ? (
                  <div className="pointer-events-none absolute inset-0 animate-pulse bg-indigo-100/70 dark:bg-slate-800/80" />
                ) : null}
                {!mediaAspectRatio && !thumbLoaded ? (
                  <div
                    className="pointer-events-none w-full animate-pulse bg-indigo-100/70 dark:bg-slate-800/80"
                    style={{ height: "180px" }}
                  />
                ) : null}
              </div>
        ...
        if (isProcessingVideo) {
          return (
            <div
              key={key}
              className="relative block w-full overflow-hidden rounded-xl border border-indigo-200/70 bg-slate-200/70 dark:border-indigo-500/30 dark:bg-slate-800/70"
            >
        ...
        if (isVideo && file.url) {
          return (
            <button
              ...
              className="relative block w-full overflow-hidden rounded-xl border border-indigo-200/70 bg-black/60 dark:border-indigo-500/30"
        ...
        return file.url ? (
          ...
          >
            <span className="relative inline-flex h-5 w-5 shrink-0 items-center justify-center">
              <File
                size={18}
                className="absolute text-indigo-600 transition-opacity duration-150 group-hover:opacity-0 dark:text-indigo-300"
              />
              <Download
                size={18}
                className="absolute text-indigo-600 opacity-0 transition-opacity duration-150 group-hover:opacity-100 dark:text-indigo-300"
              />
            </span>
          ...
          <div
            key={key}
            className={`${docChipClass} items-center gap-2 rounded-xl border border-indigo-200/70 bg-white/70 px-3 py-2.5 text-xs text-slate-700 dark:border-indigo-500/30 dark:bg-slate-900/50 dark:text-slate-200`}
          >
            <File
              size={18}
