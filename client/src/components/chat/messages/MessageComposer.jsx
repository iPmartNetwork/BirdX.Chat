...
export function MessageComposer({
  ...
  return (
    <form
      ref={composerRef}
      className="sticky bottom-0 z-30 flex shrink-0 flex-col gap-3 border-t border-slate-300/80 bg-white px-4 py-3 dark:border-indigo-500/20 dark:bg-slate-900 sm:px-6 md:static md:mt-auto"
      style={{
        bottom: isDesktop
          ? undefined
    ...
    >
      {editTarget ? (
        <div className="rounded-2xl border border-indigo-200/80 bg-indigo-50/70 p-2 dark:border-indigo-500/30 dark:bg-slate-950/70">
          <div className="flex items-start gap-3 px-1">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-200">
              <Pencil size={20} className="icon-anim-sway" />
            </div>
            <div className="flex min-w-0 flex-1 flex-col">
              <span
                className={`truncate text-[11px] font-semibold text-indigo-700 dark:text-indigo-200 ${
                  hasPersian(
                    "Edit Message",
                  )
                    ? "font-fa"
      ...
      {pendingVoiceMessage ? (
        <div className="rounded-2xl border border-indigo-200/80 bg-indigo-50/70 p-2 dark:border-indigo-500/30 dark:bg-slate-950/70">
          <div className="flex items-start gap-3 px-1">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <span className="truncate text-[11px] font-semibold text-indigo-700 dark:text-indigo-200">
                Voice message
              </span>
      ...
      {pendingUploadFiles?.length ? (
        <div className="rounded-2xl border border-indigo-200/80 bg-indigo-50/70 p-2 dark:border-indigo-500/30 dark:bg-slate-950/70">
          <div className="mb-2 flex items-center justify-between px-1 text-[11px] font-semibold text-indigo-700 dark:text-indigo-200">
            <span>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 transition ${
                  fileUploadEnabled
                    ? "border-indigo-200/70 text-indigo-700 hover:border-indigo-300 hover:bg-indigo-100 dark:border-indigo-500/30 dark:text-indigo-200 dark:hover:bg-indigo-500/10"
          ...
          <div className="chat-scroll grid max-h-40 grid-cols-2 gap-2 overflow-y-auto sm:grid-cols-3">
            {pendingUploadFiles.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="relative overflow-hidden rounded-xl border border-indigo-200/70 bg-white/90 p-2 text-[11px] dark:border-indigo-500/30 dark:bg-slate-900/70"
                >
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  {isImage ? (
                    ...
                    <div className="mb-1 flex h-24 w-full items-center justify-center rounded-md bg-slate-100 text-indigo-700 dark:bg-slate-800 dark:text-indigo-200">
                      <File size={16} />
                    </div>
                  )}
      ...
      {activeUploadProgress !== null ? (
        <div className="rounded-xl border border-indigo-200/70 bg-indigo-50/60 px-3 py-2 dark:border-indigo-500/30 dark:bg-slate-950/70">
          ...
          <div className="h-2 overflow-hidden rounded-full bg-indigo-100 dark:bg-slate-800">
            <div
      ...
      <div className="flex flex-row items-center gap-3">
        {!isRecording ? (
          <>
            <div className="relative" ref={uploadMenuRef}>
              ...
              {showUploadMenu && !uploadBusy ? (
                <div className="absolute bottom-12 left-0 z-40 w-44 rounded-xl border border-indigo-200/80 bg-white p-1.5 shadow-lg dark:border-indigo-500/30 dark:bg-slate-950">
                  <button
                    type="button"
                    onPointerDown={(event) => {
                      captureComposerFocus();
                    ...
                    className="flex w-full items-center gap-2 rounded-lg border border-transparent px-2 py-2 text-left text-xs text-indigo-700 transition hover:border-indigo-300 hover:bg-indigo-100 dark:text-indigo-200 dark:hover:border-indigo-500/30 dark:hover:bg-indigo-500/10"
                  >
                  ...
                  <button
                    ...
                    className="mt-1 flex w-full items-center gap-2 rounded-lg border border-transparent px-2 py-2 text-left text-xs text-indigo-700 transition hover:border-indigo-300 hover:bg-indigo-100 dark:text-indigo-200 dark:hover:border-indigo-500/30 dark:hover:bg-indigo-500/10"
                  >
            ...
            <textarea
              ...
              className={`chat-scroll min-w-0 flex-1 resize-none rounded-2xl border border-indigo-200 bg-white px-4 py-2 text-base text-slate-700 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-300/60 dark:border-indigo-500/30 dark:bg-slate-900 dark:text-slate-100 ${
          ...
          <div className="flex min-w-0 flex-1 items-center justify-between rounded-2xl border border-indigo-200 bg-white px-4 py-2 text-sm text-indigo-700 dark:border-indigo-500/30 dark:bg-slate-900 dark:text-indigo-200">
        ...
        <button
          ...
          className={`inline-flex h-11 items-center justify-center rounded-2xl px-4 py-2 text-sm font-semibold text-white shadow-lg transition disabled:cursor-not-allowed disabled:opacity-70 ${
            isRecording
              ...
              : "bg-indigo-500 shadow-indigo-500/30 hover:bg-indigo-400 hover:shadow-indigo-500/40"
          }`}
        >
          {micMode || isRecording ? (
